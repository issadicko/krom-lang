...
class NameMangler {
  ...
  Statement _mangleStatement(Statement stmt) {
    if (stmt is FunctionDeclaration) {
      ...
      final name =
          _mangleIdentifier(stmt.name); // Usually keeps original for global
      ...
      return VarDecl(
          stmt.token, Identifier(stmt.name.token, newName), newValue);
      ...
      return ExpressionStatement(
          stmt.token, _mangleExpression(stmt.expression));
    } else if (stmt is ReturnStatement) {
      return ReturnStatement(stmt.token,
          stmt.value != null ? _mangleExpression(stmt.value!) : null);
      ...
      return IfStatement(
          ...
          stmt.alternative != null
              ? (_mangleStatement(stmt.alternative!) as BlockStatement)
              : null);
    } else if (stmt is WhileStatement) {
      return WhileStatement(stmt.token, _mangleExpression(stmt.condition),
      ...
      final newBody = _mangleStatement(stmt.body) as BlockStatement;
      _exitScope();
      return ForStatement(stmt.token,
          Identifier(stmt.variable.token, newVarName), newIterable, newBody);
    }
  ...
  Expression _mangleExpression(Expression expr) {
    if (expr is Identifier) {
      ...
      return BinaryExpr(expr.token, _mangleExpression(expr.left), expr.operator,
          _mangleExpression(expr.right));
    } else if (expr is UnaryExpr) {
      return UnaryExpr(
          expr.token, expr.operator, _mangleExpression(expr.right));
    } else if (expr is CallExpr) {
      return CallExpr(expr.token, _mangleExpression(expr.function),
          expr.arguments.map(_mangleExpression).toList());
    } else if (expr is Assignment) {
      return Assignment(expr.token, _mangleExpression(expr.left),
          _mangleExpression(expr.value));
    } else if (expr is ArrayLiteral) {
      return ArrayLiteral(
          expr.token, expr.elements.map(_mangleExpression).toList());
      ...
      return IndexExpr(expr.token, _mangleExpression(expr.left),
          _mangleExpression(expr.index));
    } else if (expr is PropertyAccessExpr) {
      return PropertyAccessExpr(
          expr.token, _mangleExpression(expr.obj), expr.property);
    } else if (expr is SafeAccessExpr) {
      return SafeAccessExpr(
          expr.token, _mangleExpression(expr.obj), expr.property);
    } else if (expr is ElvisExpr) {
      return ElvisExpr(expr.token, _mangleExpression(expr.left),
          _mangleExpression(expr.defaultValue));
    } else if (expr is TernaryExpr) {
      return TernaryExpr(
          expr.token,
          _mangleExpression(expr.condition),
          _mangleExpression(expr.consequent),
          _mangleExpression(expr.alternate));
    } else if (expr is StringTemplate) {
      return StringTemplate(
          expr.token, expr.parts.map(_mangleExpression).toList());
    } else if (expr is FunctionLiteral) {
      _enterScope();
      final newParams = expr.parameters
          .map((p) => Identifier(p.token, _mangleName(p.value, isDecl: true)))
          .toList();
