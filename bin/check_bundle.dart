...
void main() {
  final content = File('/Users/issahamadoudicko/IdeaProjects/krom/kmini_program/example/assets/bundle.json').readAsStringSync();
  ...
  stdout.writeln('Bracket count: [ = $openBracket, ] = $closeBracket, match: ${openBracket == closeBracket}');
  stdout.writeln('Paren count: ( = $openParen, ) = $closeParen, match: ${openParen == closeParen}');
  stdout.writeln('');
  ...
  if (lines.length >= 379) {
    ...
    stdout.writeln('Script only has ${lines.length} lines, error is not in home.script');
    ...
    for (final entry in pages.entries) {
      ...
      openBracket = 0; closeBracket = 0; openParen = 0; closeParen = 0;
      ...
      stdout.writeln('Page "$pageName": ${pageLines.length} lines, brackets: ${match ? "OK" : "MISMATCH"} ([ $openBracket/$closeBracket, ( $openParen/$closeParen)');
    }
  }
}
